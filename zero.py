...
def start_attack_reply(message, target, port, time):
    ...
    response = (
        ...
        f"🎮 𝐆𝐚𝐦𝐞: `𝐁𝐆𝐌𝐈`\n\n"
        f"📝 𝐀𝐭𝐭𝐚𝐜𝐤 𝐢𝐧 𝐩𝐫𝐨𝐠𝐫𝐞𝐬𝐬... {time} seconds remaining.\n"
        f"Wait here without touching any button... 🚀"
    )
    ...
    message_sent = bot.reply_to(message, response, parse_mode='Markdown', reply_markup=keyboard)
    ...
    for remaining_time in range(time, 0, -1):
        bot.edit_message_text(
            ...
            text=(
                f"🎯 𝐓𝐚𝐫𝐠𝐞𝐭: `{target}`\n"
                f"🔌 𝐏𝐨𝐫𝐭: `{port}`\n"
                f"⏳ 𝐑𝐞𝐦𝐚𝐢𝐧𝐢𝐧𝐠 𝐓𝐢𝐦𝐞: `{remaining_time} seconds`\n"
                f"📝 𝐀𝐭𝐭𝐚𝐜𝐤 𝐬𝐭𝐢𝐥𝐥 𝐢𝐧 𝐩𝐫𝐨𝐠𝐫𝐞𝐬𝐬... Please wait."
            ),
            parse_mode='Markdown'
        )
        time.sleep(1)
